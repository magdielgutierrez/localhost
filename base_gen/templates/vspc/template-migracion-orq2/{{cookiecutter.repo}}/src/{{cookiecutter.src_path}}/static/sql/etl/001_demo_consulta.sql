...
DROP TABLE IF EXISTS {zona_p}.{indice}temporal_ads_package_gen PURGE;
CREATE TABLE IF NOT EXISTS {zona_p}.{indice}temporal_ads_package_gen STORED AS PARQUET TBLPROPERTIES ('transactional'='false') AS
    SELECT
        tipo_doc,
		num_doc,
		p_nombre,
    ...
    LIMIT
        10;
COMPUTE STATS {zona_p}.{indice}temporal_ads_package_gen;
--------------------------------- Query End ---------------------------------